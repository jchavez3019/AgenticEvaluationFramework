...
- **Project layout:** `src/`-layout, package name `aef` (Agentic Evaluation Framework). The backend lives at `backend/` (with `backend/pyproject.toml`, `backend/uv.lock`, `backend/src/aef/`, `backend/tests/`). `configs/`, `outputs/`, `Makefile`, `.pre-commit-config.yaml`, `.github/workflows/`, and `frontend/` are repo-root peers of `backend/`. The CLI resolves the `configs/` directory via a workspace-root anchor (Hydra's `config_path` relative to the script entry point).
...
- **Affected paths**:
  ...
  - `backend/uv.lock` — committed lockfile generated by `uv lock`.
  - `backend/src/aef/` — package root following the layout in `high_level_architecture.md` §3.1.
  - `backend/src/aef/api/app.py` — FastAPI app factory.
  - `backend/src/aef/persistence/models.py` — SQLAlchemy 2.x typed declarative base and ORM classes.
  - `backend/src/aef/persistence/migrations/` — Alembic environment (`env.py`, `script.py.mako`, `versions/`).
  - `backend/src/aef/contracts/` — Pydantic models for `EvaluationRunRequest`, `EvaluationRunResult`, `EvaluationSample`, `GenerationRequest`, `GenerationResponse`, `MetricResult`, adapter specs.
  - `backend/README.md` — short bootstrap instructions for running `uv sync`, `uv run`, `alembic upgrade head`, and `uvicorn aef.api.app:app`.
