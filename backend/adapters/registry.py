...
``backend.adapters`` package import triggers all of them.

Third-party packages can supply additional adapters via the
``backend.adapters.models``, ``backend.adapters.judges``, and
``backend.adapters.datasets`` Python entry-point groups (lazy discovery on
first lookup).
...
class _Registry[SpecT, AdapterT]:
    ...
    def __init__(self, entry_point_group: str) -> None:
        ...
        :param entry_point_group: ``importlib.metadata`` group for lazy discovery.
        """
    ...
    def register(
        ...
        :param name: Adapter name (must match ``spec.name`` at build time).
        :param factory: Callable ``(spec) -> adapter`` for this name.
        """
        with self._lock:
    ...
    def build(self, name: str, spec: SpecT) -> AdapterT:
        ...
        :param spec: Typed adapter specification for the factory.

        :return: Concrete adapter instance.
        """
        with self._lock:
    ...
    def unregister(self, name: str) -> None:
        ...
        Exposed primarily so tests can clean up registrations they made for the duration of a
        test without leaking state across the rest of the suite.

        :param name: Registry name for the adapter or metric.
        """
        with self._lock:
            self._factories.pop(name, None)

    def _discover_entry_points_locked(self) -> None:
        """Load metric plugins from entry points (locked)."""
        if self._discovered:
            return
...
def register_model_adapter(
    ...
    :param name: Registry name for the adapter or metric.
    :param factory: Callable that constructs an implementation from ``spec``.
    """
...
def build_model_adapter(spec: ModelAdapterSpec) -> ModelAdapter:
    ...
    :return: :class:`ModelAdapter` instance.
    """
...
def build_judge_adapter(spec: JudgeAdapterSpec) -> JudgeAdapter:
    ...
    :return: :class:`JudgeAdapter` instance.
    """
...
def build_dataset_adapter(spec: DatasetAdapterSpec) -> DatasetAdapter:
    ...
    :return: :class:`DatasetAdapter` instance.
    """
