...
class UnsupportedSamplingParameterError(ValueError):
    ...
    def __init__(
        ...
        :param unsupported: parameters the request set that the adapter cannot honor.
...
class ContextOverflowError(ValueError):
    ...
    def __init__(
        ...
        :param adapter_name: Human-readable adapter name for error messages.
        :param prompt_tokens: Prompt token count for context validation.
        :param max_output_tokens: Maximum output tokens the adapter may emit.
...
def validate_against_capabilities(
    *,
    ...
    Two checks happen:  1. Every sampling field the user explicitly set (i.e., not ``None``)
    must appear in :attr:`ModelCapabilities.supported_sampling_parameters`. Unsupported
    parameters raise :class:`UnsupportedSamplingParameterError`. 2. When the adapter
    declares :attr:`ModelCapabilities.max_context_tokens` and ``prompt_tokens`` is supplied,
    the sum of ``prompt_tokens`` + ``sampling.max_output_tokens`` must not exceed the limit.
    Overflow raises :class:`ContextOverflowError`.  :raises
    UnsupportedSamplingParameterError: see check 1. :raises ContextOverflowError: see check
    2.

    :param adapter_name: Human-readable adapter name for error messages.
    :param capabilities: Declared model capabilities.
