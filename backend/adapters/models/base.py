...
class ModelAdapter(Protocol):
    ...
    def spec(self) -> ModelAdapterSpec:
        ...
        :return: :class:`ModelAdapterSpec` instance.
        """
        ...
    ...
    async def generate(
        ...
        :raises UnsupportedSamplingParameterError: when ``request.sampling`` sets a parameter
        the adapter does not advertise. :raises ContextOverflowError: when the adapter can count
        tokens and the request exceeds its context window.
...
class JudgeAdapter(ModelAdapter, Protocol):
    ...
    def spec(self) -> JudgeAdapterSpec:
        ...
        :return: :class:`JudgeAdapterSpec` instance.
        """
        ...
    ...
    async def judge(self, request: JudgmentRequest) -> JudgmentResponse:
        ...
        Bias mitigations (position swap for pairwise judges, anchors, determinism) are applied
        by the metric layer using fields on :attr:`JudgmentRequest.bias_mitigation`.

        :param request: Judgment request (candidate + rubric).

        :return: Structured :class:`JudgmentResponse`.
        """
        ...
