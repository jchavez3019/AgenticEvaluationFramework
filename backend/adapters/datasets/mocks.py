...
from backend.adapters.registry import register_dataset_adapter
...
def _seeded_rows(count: int) -> list[EvaluationSample]:
    ...
    The arithmetic answers are deliberately exact so lexical metrics (BLEU / exact-match) on
    a pass-through adapter produce well-understood reference values.

    :param count: Number of seeded rows to synthesize.

    :return: ``count`` deterministic arithmetic Q&A rows (cycled when ``count`` > 5).
    """
    inputs = [
        ("What is 1+1?", "2"),
        ("What is 2+2?", "4"),
...
class MockDatasetAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        :param spec: :class:`DatasetAdapterSpec` for this adapter.
        :param rows: Explicit rows; defaults to five seeded arithmetic samples.
        """
        self.spec = spec
    ...
    async def load(self) -> AsyncIterator[EvaluationSample]:
        ...
        :yields: The configured dataset rows in index order.
        """
...
def _factory(spec: DatasetAdapterSpec) -> MockDatasetAdapter:
    ...
    :param spec: Dataset adapter specification.

    :return: Configured :class:`MockDatasetAdapter` instance.
    """
    return MockDatasetAdapter(spec)


def _register() -> None:
    """Register ``mock`` with the dataset adapter registry (idempotent)."""
    try:
