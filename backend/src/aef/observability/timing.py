...
from aef.observability.context import current_context
...
class TelemetryRecorder:
    ...
    def record(self, run_id: str | None, record: TimingRecord) -> None:
        ...
        :param run_id: the run id from :func:`current_context`. When
            ``None``, the record is dropped silently.
    ...
    def reset(self, run_id: str | None = None) -> None:
        ...
        Tests use this to keep recorder state from leaking between
        cases. Production code never calls it — runs finalize via
        :meth:`dump_run`.
        """
        with self._lock:
            if run_id is None:
    ...
    def dump_run(self, run_id: str) -> TelemetryReport:
        ...
        Per-stage rollups (p50/p95/p99) are derived from
        :attr:`per_record`; the recorder does not pre-aggregate.

        :param run_id: identifies the run; must have at least one
            recorded entry.
        :returns: a fully-populated :class:`TelemetryReport`.
        :raises KeyError: when no entries were recorded for the run.
        """
        with self._lock:
...
def get_recorder() -> TelemetryRecorder:
    ...
    Tests inspect / reset this instance via the ``caplog_aef`` fixture
    family in :mod:`backend.tests.conftest`.
    """
    return _RECORDER
...
def timed(phase: str) -> _Timed:
    ...
    Usage as a context manager::
        ...
        with timed("metric.bleu"):
            ...

    Usage as an async context manager::

        async with timed("generation"):
            ...

    Usage as a decorator (sync or async)::

        @timed("dataset.load")
        async def load() -> ...: ...

    The recorded :class:`TimingRecord` automatically picks up the
    ``run_id`` / ``sample_idx`` / ``stage`` from the surrounding
    :func:`run_context`.

    :param phase: human-readable phase label. Conventional values:
        ``generation``, ``metric.<name>``, ``dataset.load``,
        ``persist``, ``judge.<name>``.
    :returns: a :class:`_Timed` instance that is both context manager
        and decorator factory.
    """
    return _Timed(phase)
