...
from aef.adapters.capabilities import validate_against_capabilities
from aef.adapters.registry import register_judge_adapter, register_model_adapter
...
class MockChatModel:
    ...
    The adapter satisfies the :class:`ModelAdapter` Protocol structurally;
    the registry returns it by ``"mock-chat"``.

    :param spec: adapter spec; consumed for ``capabilities`` validation.
    :param scripts: ordered list of :class:`MockChatScript` rules.
    :param seed: included for parity with real adapters (does not affect
        the mock's output today; future randomized scripts will read it).
    :param sleep_for_latency: when ``True``, the adapter awaits
        ``asyncio.sleep`` to simulate latency. Tests that don't want to
        block the event loop can pass ``False``.
    """
    ...
    def __init__(
        ...
        *,
        ...
        """Hold the spec / scripts / seed for later :meth:`generate` calls."""
        self.spec = spec
...
class MockJudge:
    ...
    async def generate(
        ...
        The judge metric layer calls :meth:`judge`; this method exists
        only because :class:`JudgeAdapter` extends :class:`ModelAdapter`.
        """
        validate_against_capabilities(
            adapter_name=self.spec.name,
            capabilities=self.spec.capabilities,
...
def _build_default_chat_scripts() -> list[MockChatScript]:
    ...
    The factory stores empty scripts; tests pass custom scripts through
    a wrapper because the registry receives only the spec.
    """
...
def _model_factory(spec: ModelAdapterSpec) -> MockChatModel:
    ...
    The default scripts are intentionally minimal — tests build a
    :class:`MockChatModel` directly when they need scripted behavior.
    """
