...
- :mod:`aef.contracts.primitives` — leaf shapes (chat, generation,
  sample, judge, engine, output) with no sibling imports.
- :mod:`aef.contracts.adapter_spec` — adapter specs (depends on
  primitives).
- :mod:`aef.contracts.metric_result` — metric shapes (depends on
  primitives + adapter_spec).
- :mod:`aef.contracts.telemetry` — telemetry shapes (depends on
  primitives).
- :mod:`aef.contracts.run` — top-level run request/result (depends on
  every other module above).
- :mod:`aef.contracts.persistence` — record models for
