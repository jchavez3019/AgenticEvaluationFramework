...
class Metric(Protocol):
    ...
    async def compute_batch(
        ...
        :return: One :class:`MetricResult` per input element.
        """
        ...
...
class BaseMetric:
    ...
    def __init__(self, spec: MetricSpec) -> None:
        """Hold the metric spec used for naming, versioning, and configuration.

        :param spec: Registered :class:`MetricSpec` for this instance.
        """
        self.spec = spec

    @abstractmethod
    def _score(self, inputs: MetricInputs) -> tuple[float | None, list[SubScore]]:
        """Compute the primary scalar and structured sub-scores for one sample.

        Subclasses implement the core metric logic; :meth:`compute` wraps this
        with timing, status handling, and :class:`MetricResult` assembly.

        :param inputs: Per-sample metric inputs.
        :return: ``(value, sub_scores)`` where ``value`` may be ``None`` when skipped.
        """
    ...
    async def aggregate(
        ...
        Skipped or errored samples are ignored. Run-level metrics that cannot be reduced to a
        mean (Self-BLEU, retrieval recall over a per-run set) override this method.
...
def metric_factory(metric_cls: type[BaseMetric]) -> Callable[[MetricSpec], Metric]:
    ...
    :param metric_cls: Concrete metric class constructed by the factory.

    :return: Factory callable accepted by :func:`register_metric`.
    """
    ...
    def _factory(spec: MetricSpec) -> Metric:
        ...
        :param spec: :class:`MetricSpec` passed to the constructor.

        :return: Concrete :class:`Metric` instance.
        """
