...
from backend.observability.context import ContextvarsFilter

_ROOT_LOGGER_NAME = "backend"
_CONSOLE_HANDLER_NAME = "backend.console"
_FILE_HANDLER_NAME = "backend.file"
...
def get_logger(name: str) -> logging.Logger:
    ...
    The returned logger inherits the handlers attached to the root ``backend`` logger by
    :func:`configure_logging`. Module-level loggers have NO handlers attached directly; they
    propagate up. Calling this repeatedly is safe and cheap.
...
class _JsonFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        """
        Format a :class:`logging.LogRecord` for output.

        :param record: Log record to format.

        :return: Single JSON log line for the record.
        """
        import json
...
def _make_formatter(fmt: LogFormat) -> logging.Formatter:
    """
    Return the :class:`logging.Formatter` for ``fmt``.

    :param fmt: Logging output format selector.
...
def configure_logging(settings: LoggingSettings | None = None) -> None:
    ...
    Idempotent: calling this twice in the same process replaces the existing handlers rather
    than duplicating them. The CLI / API process call this once during startup; tests may
    call it inside a fixture.

    :param settings: typed configuration; defaults to console output at ``INFO`` level when
                     omitted.
    """
    if settings is None:
