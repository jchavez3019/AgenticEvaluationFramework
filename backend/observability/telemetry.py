...
class _NoOpSpan(AbstractContextManager["_NoOpSpan"]):
    ...
    def __init__(self, name: str, attributes: dict[str, object]) -> None:
        ...
        :param name: Span name (for example ``metric.bleu``).
        :param attributes: Key-value attributes stored on the no-op span.
        """
        self.name = name
...
def start_span(name: str, **attributes: object) -> AbstractContextManager[_NoOpSpan]:
    ...
    The current implementation never imports OpenTelemetry. When ``AEF_OTEL_ENABLED=1`` is
    set, a future plan replaces the body of this function with a real OTLP span emission;
    call sites stay the same.

    :param name: span name (e.g., ``"metric.bleu"``, ``"engine.generation_stage"``).

    :return: a context manager; entering / exiting the block records nothing today.
    """
