...
def redact_secrets(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    :data:`SENSITIVE_FIELD_SUBSTRINGS` are replaced with the redaction placeholder. Nested
    dicts and lists are walked recursively. The function is intentionally conservative — it
    never mutates the original payload, and it prefers false positives (over-redacting) to
    under-redacting, since the cost of leaking a secret outweighs the cost of redacting one
    too many fields.

    :param payload: JSON-like payload to redact.

    :return: :class:`Any` instance.
    """
...
def _redact(value: Any) -> Any:  # — heterogeneous JSON payload.
    """
    Recursively redact sensitive fields in ``value``.
...
def _is_sensitive(key: str) -> bool:
    ...
    :param key: Field name to test for sensitivity.
...
class StorageAdapter(Protocol):
    ...
    async def upsert_model_metadata(self, meta: ModelMetadataRecord) -> None:
        ...
        :param meta: Metadata record to upsert.
        """
        ...
